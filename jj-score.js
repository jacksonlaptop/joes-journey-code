/* jj-score.js — site-wide stars + coins + achievements (build k50: Rise and Shine (prod the sleeping Trogdor); k49: Special ring on pills/transport/sound matches the CTAs; the Storytime hint box; k48: the moon sound button follows the theme; k47: Forest Friend achievement (the tree spirit); k46: My Story's NEXT wears the HUD pill, theme for theme; k45: no stars on the CTA, Space CTA matches the Space panel; classic modals more transparent; classic story prompts wear the panel glass, every overlay blurs, the nav is dead under all of them; Themes opens over the menu, Store/Achievements close it first; panel back up, waves stretched not tiled, tab dot on the corner, rows hug their content, payouts fly above everything; the wave strip tiles at half the viewport so the crests read right across it; nav behind + unclickable under modals, waves fixed to the viewport; the panel never rises into the nav; combination achievements catch up on load; nav above modals again, new credits/contact art, CTA glyph only on the label; dressing moved onto the panel (waves truly full width, behind the card); the panel's 8-bit Trogdor flies off into the distance and back like the companion, Special waves centred full-width; pink press clipped to its button, classic tabs on the glass rule; the secret portal achievement (5 stars + the big well-done), Part Two gate 'tale2'; one achievement per tale (theme + star), owned/done counts in the header, classic = the click-to-begin glass everywhere, Special stone buttons, medieval rows spaced, tab fade, PREVIEW_RICH; story prompt buttons sized down; frames outside rows, waves full width, Special CTA, retro tab hover; Special dressed (trog, alien, moon, waves) + parchment collected cards; narrower card, tick aligned, preview themes on; wallet + spend, daily bonus, store registry for jj-companion, music themes; locked themes shake instead of jumping).
...
(function () {
  ...
  var GB = window.JJ_SCORE_BASE || 'https://cdn.jsdelivr.net/gh/jacksonlaptop/joes-journey-code@main/';   // JJ_SCORE_BASE: local preview override
  ...
  var SND = { open: GB + 'jj-menu-open.mp3', close: GB + 'jj-menu-close.mp3', coin: GB + 'jj-coin-earn.mp3', spend: GB + 'jj-coin-spend.mp3', star: GB + 'jj-star.mp3', theme: GB + 'jj-theme.mp3' };   // files pending — silent until they exist                                // menu scroll sounds (files pending — silent until they exist)
  ...
  /* ---- the achievements (names + copy are placeholders until the proper list lands). kind: star | coin | theme.
         icon: ach-<lowercase-id>.png on the repo root when it exists, else the tab's icon. ---- */
  var ACH = [
    { id: 'scroll',        tab: 'general',   kind: 'theme', theme: 'alien',    star: true, name: 'Journeyman',  desc: 'Make it to the end of the journey: a star, and the Space theme' },
    { id: 'alien-catch',   tab: 'general',   kind: 'coin',  name: 'Cosmic Catch',  desc: 'Catch the shifty alien peeking in on the journey' },
    { id: 'quiz80',        tab: 'general',   kind: 'coin',  name: 'Top of the Class', desc: 'Score 80% or more in the quiz' },
    { id: 'allrounder',    tab: 'general',   kind: 'theme', theme: 'mixed',    name: 'A Bit of Everything', desc: 'Five in one, each fills a bubble: defeat Trogdor, reach level 6 in the credits game, catch the shifty alien on the journey, score 80%+ in the quiz, and connect on LinkedIn. All five unlock the Special theme', needs: ['trogdor', 'credits5', 'alien-catch', 'quiz80', 'linkedin'] },
    { id: 'planets',       tab: 'general',   kind: 'coin',  name: 'Planet Hunter',    desc: 'Click Jupiter or Mars' },
    ...
    { id: 'babadook',      tab: 'general',   kind: 'coin',  name: 'Ba-ba-ba\u2026 DOOK!', desc: 'Press play on the secret video at the end of the quiz. Unlocks the Babadook alien' },
    { id: 'big-bangs',     tab: 'general',   kind: 'star',  name: 'Two Big Bangs',    desc: 'Watch both Big Bangs on the home intro and in My Story', target: 2 },
    { id: 'sound-on',      tab: 'general',   kind: 'coin',  reward: 20, name: 'Sound Decision', desc: 'Begin the journey with the sound left on' },
    { id: 'storytime',     tab: 'storytime', kind: 'theme', theme: 'medieval', star: true, name: 'The Tale of Trogdor & Joe the Righteous', desc: 'Watch the whole animated tale: a star, and the Medieval theme' },
    { id: 'story-secrets', tab: 'storytime', kind: 'star',  name: 'Joe the Righteous', desc: 'Find everything in The Tale of Trogdor & Joe the Righteous' },
    { id: 'bones',         tab: 'storytime', kind: 'coin',  name: 'Bone Collector',   desc: 'Pick up all the bones in Storytime', target: 2 },
    { id: 'treasure',      tab: 'storytime', kind: 'coin',  name: 'Treasure Collector', desc: 'Open the treasure chest in Storytime' },
    { id: 'chick-cave',    tab: 'storytime', kind: 'coin',  name: 'Ruffled Feathers', desc: 'Give the shivering chicken in the cave a prod. Unlocks the Armoured Chicken in the store' },
    { id: 'chick-village', tab: 'storytime', kind: 'coin',  name: 'Chicken Run',      desc: 'Startle the three chickens outside the village house. Unlocks the Battle Chicken in the store' },
    { id: 'spirit',        tab: 'storytime', kind: 'coin',  name: 'Forest Friend',    desc: 'Say hello to a tree spirit in the enchanted forest. Unlocks the Tree Spirit in the store' },
    { id: 'wake',          tab: 'storytime', kind: 'coin',  name: 'Rise and Shine',   desc: 'Prod Trogdor while he sleeps in his cave. Brave, or foolish' },
    ...
    { id: 'tale2',         tab: 'storytime', kind: 'coin',  needs: ['storytime', 'quiz'], name: 'Part Two', desc: 'Finish The Tale of Trogdor & Joe the Righteous, then the quiz in My Story, and Part Two of the tale opens in the menu' },
    { id: 'portal',        tab: 'storytime', kind: 'star',  stars: 5, secret: true, name: 'Secret Achievement', desc: 'Hidden somewhere in the tale. Keep your eyes open, and your cursor special.', doneName: 'Through the Portal', doneDesc: 'You opened the portal with the Special cursor. Five stars, and the tale is truly complete.' },
    { id: 'casestudies',   tab: 'work',      kind: 'coin',  name: 'Case Closed',      desc: 'Read all three case studies', target: 3 },
    ...
    { id: 'credits5',      tab: 'credits',   kind: 'coin',  name: 'Getting Stronger', desc: 'Reach level 6 in the credits game' },
    { id: 'credits',       tab: 'credits',   kind: 'theme', theme: 'retro',    star: true, name: 'Credits Complete', desc: 'Play the credits to the end: a star, and the Retro theme' },
    { id: 'credits10',     tab: 'credits',   kind: 'star',  name: 'Maxed Out',        desc: 'Reach level 9 in the credits game' }
  ];
  ...
  var PREVIEW_THEMES = true;   // TESTING: locked themes still switch (they keep the locked look). Set false to enforce the unlocks.
  var MUSIC = [ { id: 'classic', name: 'Classic', src: 'https://cdn.prod.website-files.com/6a19b8f4191d4fbca532591e/6a19b8f4191d4fbca53259a5_Lotro-ambient.mp3' },
                { id: 'medieval', name: 'Medieval', src: GB + 'music-medieval.mp3', by: 'storytime' }, { id: 'alien', name: 'Space', src: GB + 'music-space.mp3', by: 'scroll' }, { id: 'retro', name: '8-bit', src: GB + 'game-music-1.mp3', by: 'credits' } ];   // the site's ambient, swapped through jjAudio.swapAmbient; medieval + space tracks pending
  function musicOk(id) { var m = MUSIC.filter(function (x) { return x.id === id; })[0]; return !!m && (!!window.JJ_LOCAL_PREVIEW || !m.by || !!S.done[m.by]); }   // …and every track
  function applyMusic() { var m = MUSIC.filter(function (x) { return x.id === (S.music || 'classic'); })[0]; if (!m) return; var tries = 0; (function go() { if (window.jjAudio && window.jjAudio.swapAmbient) window.jjAudio.swapAmbient(m.src); else if (tries++ < 80) setTimeout(go, 250); })(); }
  function setMusic(id) { if (!musicOk(id)) return false; S.music = id; save(); applyMusic(); sfx('theme'); return true; }
  var BY = {}; ACH.forEach(function (a) { a.target = a.needs ? a.needs.length : (a.target || 1); BY[a.id] = a; });
  function givesStar(a) { return a.kind === 'star' || !!a.star; }   // the animated tales pay a star with their theme
  var STAR_MAX = ACH.filter(givesStar).reduce(function (n, a) { return n + (a.stars || 1); }, 0);
  var FIRST = {   // first-time explainer copy (placeholder)
    custom: { title: 'Time to customise', body: 'That one goes on your companion. Open the Customise tab in the Store to dress them: tap a slot, pick what you own, and they wear it everywhere you go.' },
    ...
    secret: { title: 'Well done!', body: 'You found the secret achievement. Five stars are yours, and the tale is truly complete.', img: GB + 'story-portal-4.webp' }
  };
  ...
  function shape() { S.p = S.p || {}; S.parts = S.parts || {}; S.done = S.done || {}; S.seen = S.seen || {}; S.seenAch = S.seenAch || {}; S.theme = S.theme || 'classic'; S.spent = S.spent || { c: 0, s: 0 }; S.bonus = S.bonus || 0; }
  shape(); themeAttr();
  ['storytime', 'scroll', 'credits'].forEach(function (id) { var old = S.done[id + '-star']; if (old && !S.done[id]) { S.done[id] = old; S.p[id] = 1; } delete S.done[id + '-star']; });   // k31: the star halves merged into the theme rows
  ...
  function earnedStars() { return ACH.filter(function (a) { return givesStar(a) && S.done[a.id]; }).reduce(function (n, a) { return n + (a.stars || 1); }, 0); }
  function earnedCoins() { return ACH.filter(function (a) { return a.kind === 'coin' && S.done[a.id]; }).reduce(function (n, a) { return n + (a.reward || COIN); }, 0) + (S.bonus || 0); }
  function stars() { return Math.max(0, earnedStars() - ((S.spent && S.spent.s) || 0)); }      // the wallet: what is left to spend (the achievements rows keep the history)
  function coins() { return Math.max(0, earnedCoins() - ((S.spent && S.spent.c) || 0)); }
  var PREVIEW_RICH = true;                                  // TESTING: the store never says no and never deducts. Set false to go live.
  function spend(kind, n) {                                  // the store pays with this; false = can't afford
    var star = kind === 'star', have = star ? stars() : coins(); n = n || 0; if (PREVIEW_RICH) { sfx('spend'); return true; } if (n > have) return false;
    var el = star ? nStar : nCoin, ic = star ? icStar : icCoin, from = have; S.spent[star ? 's' : 'c'] += n; save();
    if (el) { countTo(el, from, have - n, 360); } if (ic) { ic.classList.remove('jiggle', 'shine'); void ic.offsetWidth; ic.classList.add('jiggle'); }
    if (totEl) { totEl.querySelector('.ts').textContent = stars(); totEl.querySelector('.tc').textContent = coins(); }
    sfx('spend'); try { window.dispatchEvent(new CustomEvent('jj:spend', { detail: { kind: kind, n: n, stars: stars(), coins: coins() } })); } catch (e) {}
    return true;
  }
  function credit(n, label, x, y) {                          // coins from outside the achievements (the daily bonus)
    S.bonus = (S.bonus || 0) + n; save(); x = x == null ? window.innerWidth / 2 : x; y = y == null ? Math.min(160, window.innerHeight * .25) : y;
    floatAt(x, y, '+' + n + ' <img src="' + ico('coin') + '" alt=""> ' + (label || '')); fly('coin', x, y + 30, function () { land('coin'); });
  }
  function unseen() { return ACH.filter(function (a) { return S.done[a.id] && !S.seenAch[a.id]; }).length; }
  function tabNew(id) { return ACH.some(function (a) { return a.tab === id && S.done[a.id] && !S.seenAch[a.id]; }); }
  function seeTab(id) { var changed = false; ACH.forEach(function (a) { if (a.tab === id && S.done[a.id] && !S.seenAch[a.id]) { S.seenAch[a.id] = true; changed = true; } }); if (changed) save(); }
  function themeOk(t) { var th = THEMES.filter(function (x) { return x.id === t; })[0]; return !!th && (!!window.JJ_LOCAL_PREVIEW || !th.by || !!S.done[th.by]); }   // the local preview has every theme unlocked
  ...
  var ACH_ART = {
    scroll: 'joe-evo-7.webp', 'scroll-star': 'joe-evo-7.webp', 'alien-catch': 'ach-alien-catch.webp', quiz80: 'ach-quiz80.webp', allrounder: 'ach-allrounder.webp', planets: 'ach-planets.webp',
    alien: 'alien-sit-idle-poster.webp', quiz: 'ach-quiz.webp', quizFull: 'ach-quizfull.webp', babadook: 'ach-quizfull.webp', storytime: 'ach-storytime.webp', bones: 'ach-bones.webp',
    'big-bangs': 'ach-allrounder.webp', 'sound-on': 'ach-alien.webp', 'storytime-star': 'ach-storytime.webp', 'story-secrets': 'ach-storytime.webp', 'chick-cave': 'co-chick-armour.webp', 'chick-village': 'co-chick-battle.webp', spirit: 'co-spirit.webp', wake: 'co-trogdor-loop-poster.webp',
    invasion: 'ach-invasion.webp', trogdor: 'ach-trogdor.webp', treasure: 'ach-treasure.webp', linkedin: 'ach-linkedin.webp', contact: 'ach-contact.webp', cv: 'ach-cv.webp',
    'credits-click': 'ach-credits-click.webp', credits5: 'ach-credits5.webp', credits: 'ach-credits.webp', credits10: 'ach-credits10.webp'
  };
  function iconOf(a) { return { src: GB + (ACH_ART[a.id] || ('ach-' + a.id.toLowerCase() + '.png')), fb: GB + 'score-tab-' + (TABICON[a.tab] || 'general') + '-on.webp' }; }
  ...
  var css = document.createElement('style'); css.textContent =
    ...
    '#jj-sc-hud .jn::before,#jjst-hint::before,#jjst-ctl .jb::before,#jjms-next::before{content:"";position:absolute;inset:0;z-index:0;border-radius:24.5px;background:rgba(0,0,0,.4);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.5);overflow:hidden;box-sizing:border-box;transition:background .25s ease,filter .25s ease;}' +
    '#jj-sc-hud .jn::after{content:"";position:absolute;inset:0;z-index:0;border-radius:24.5px;background:rgba(0,0,0,.92);transform:scaleY(0);transform-origin:bottom center;transition:transform .4s cubic-bezier(.4,0,.2,1);pointer-events:none;}#jj-sc-hud .jn:hover::after{transform:scaleY(1);}html:not([data-jj-theme="classic"]) #jj-sc-hud .jn::after{display:none;}' +
    '#jj-sc-hud .jn:hover::before,#jjst-hint:hover::before,#jjst-ctl .jb:hover::before,#jjms-next:hover::before{background:rgba(0,0,0,.4);}#jj-sc-hud .jn-th{width:auto;padding:0 14px;gap:8px;justify-content:flex-start;flex:0 0 auto;}#jj-sc-hud .jn-st{width:56px;padding:0;justify-content:center;}#jj-sc-hud .jn-st svg{width:24px;height:23px;display:block;}' +
    '#jj-sc-hud .jn-st{isolation:isolate;overflow:visible;}#jj-sc-hud .jn-st .store-peek{position:absolute;z-index:-1;width:48px;height:48px;max-width:none;left:4px;top:-28px;object-fit:contain;pointer-events:none;opacity:0;visibility:hidden;transform:rotate(-9deg);transition:transform .3s ease,top .3s ease,opacity .25s ease;}html[data-jj-theme="alien"] #jj-sc-hud .jn-st .store-peek{opacity:1;visibility:visible;}html[data-jj-theme="alien"] #jj-sc-hud .jn-st:hover .store-peek,html[data-jj-theme="alien"] #jj-sc-hud .jn-st:focus-visible .store-peek{top:-32px;transform:rotate(3deg);}@media(prefers-reduced-motion:reduce){#jj-sc-hud .jn-st .store-peek{transition:none;}}' +
    '#jj-sc-hud .jn-sc{min-width:132px;padding:0 12px;gap:6px;}#jj-sc-hud .n{min-width:1ch;text-align:right;display:inline-block;transition:transform .2s ease;}#jj-sc-hud .n.up{transform:scale(1.35);}' +
    '#jj-sc-hud .ic{width:24px;height:22px;object-fit:contain;display:block;margin:0;transform-origin:50% 50%;}#jj-sc-hud .ic.coin{width:23px;}' +
    ...
    'html[data-jj-theme="medieval"] #jj-sc-hud .jn::before,html[data-jj-theme="medieval"] #jjst-hint::before,html[data-jj-theme="medieval"] #jjst-ctl .jb::before,html[data-jj-theme="medieval"] #jjms-next::before,html[data-jj-theme="mixed"] #jj-sc-hud .jn::before,html[data-jj-theme="mixed"] #jjst-hint::before,html[data-jj-theme="mixed"] #jjst-ctl .jb::before,html[data-jj-theme="mixed"] #jjms-next::before{border-radius:0;border:12px solid transparent;border-image:url(' + IMG.block + ') 70 fill / 12px / 0 round;background:none;backdrop-filter:none;-webkit-backdrop-filter:none;}html[data-jj-theme="medieval"] #jj-sc-hud .jn,html[data-jj-theme="mixed"] #jj-sc-hud .jn,html[data-jj-theme="mixed"] #jjms-next{font-weight:700;text-shadow:0 1px 2px rgba(0,0,0,.6);}html[data-jj-theme="medieval"] #jj-sc-hud .jn:hover::before,html[data-jj-theme="medieval"] #jjst-hint:hover::before,html[data-jj-theme="medieval"] #jjst-ctl .jb:hover::before,html[data-jj-theme="medieval"] #jjms-next:hover::before,html[data-jj-theme="mixed"] #jj-sc-hud .jn:hover::before,html[data-jj-theme="mixed"] #jjst-hint:hover::before,html[data-jj-theme="mixed"] #jjst-ctl .jb:hover::before,html[data-jj-theme="mixed"] #jjms-next:hover::before{background:none;filter:brightness(1.18);}' +
    'html[data-jj-theme="alien"] #jj-sc-hud .jn::before,html[data-jj-theme="alien"] #jjst-hint::before,html[data-jj-theme="alien"] #jjst-ctl .jb::before,html[data-jj-theme="alien"] #jjms-next::before{background:rgba(16,22,80,.82);border:1.5px solid rgba(120,220,255,.85);box-shadow:0 0 12px rgba(79,227,255,.25);}html[data-jj-theme="alien"] #jj-sc-hud .jn:hover::before,html[data-jj-theme="alien"] #jjst-hint:hover::before,html[data-jj-theme="alien"] #jjst-ctl .jb:hover::before,html[data-jj-theme="alien"] #jjms-next:hover::before{background:rgba(24,32,110,.95);}' +
    /* the moon sound button follows the theme too: a dark-and-gold ring on Medieval, the Space glow, a pixel block on Retro, the dashed ring on Special */
    'html[data-jj-theme="medieval"] #jj-sound-btn{box-shadow:0 0 0 3px #3a2a12,0 0 0 6px #c9a85c!important;}' +
    'html[data-jj-theme="alien"] #jj-sound-btn{box-shadow:0 0 0 1.5px rgba(120,220,255,.85),0 0 16px rgba(79,227,255,.5)!important;}' +
    'html[data-jj-theme="retro"] #jj-sound-btn{border-radius:10px!important;box-shadow:0 0 0 3px #FFD400,6px 6px 0 rgba(0,0,0,.35)!important;}html[data-jj-theme="retro"] #jj-sound-btn .jj-sound-fill{border-radius:10px!important;}' +
    'html[data-jj-theme="mixed"] #jj-sound-btn{outline:4px dashed #a8a8a8;outline-offset:1px;box-shadow:0 0 0 1px #6d6d6d,0 0 0 7px #6d6d6d!important;}' +
    'html[data-jj-theme="mixed"] #jjst-ctl .jb::before,html[data-jj-theme="mixed"] #jj-sc-hud .jn::before,html[data-jj-theme="mixed"] #jjms-next::before,html[data-jj-theme="mixed"] #jjst-hint::before{box-shadow:inset 0 0 0 2px #6d6d6d,0 0 0 2px #6d6d6d!important;}' +   // the Special ring exactly as the CTAs wear it
    'html[data-jj-theme="retro"] #jj-sc-hud .jn::before,html[data-jj-theme="retro"] #jjst-hint::before,html[data-jj-theme="retro"] #jjst-ctl .jb::before,html[data-jj-theme="retro"] #jjms-next::before{border-radius:0;border:12px solid transparent;border-image:url(' + RETRO.pill + ') 16 fill / 12px / 0 round;image-rendering:pixelated;background:none;backdrop-filter:none;-webkit-backdrop-filter:none;}html[data-jj-theme="retro"] #jj-sc-hud .jn,html[data-jj-theme="retro"] #jjms-next{font-family:"Mario","Joes Journey Headline",sans-serif;letter-spacing:0;}html[data-jj-theme="retro"] #jj-sc-hud .jn:hover::before,html[data-jj-theme="retro"] #jjst-hint:hover::before,html[data-jj-theme="retro"] #jjst-ctl .jb:hover::before,html[data-jj-theme="retro"] #jjms-next:hover::before{background:none;border-image-source:url(' + RETRO.pill + ');filter:brightness(1.18) drop-shadow(3px 3px 0 rgba(255,0,245,.8));}html[data-jj-theme="retro"] img[data-ico]{image-rendering:pixelated;}' +
    ...
    '.jj-sc-float{position:fixed;z-index:2147483400;pointer-events:none;display:flex;align-items:center;gap:6px;font-family:' + FONT + ';font-weight:700;color:#fff;font-size:22px;text-shadow:0 2px 6px rgba(0,0,0,.6);transform:translate(-50%,-50%);animation:jjScFloat 1.3s ease-out forwards;white-space:nowrap;}' +
    ...
    '.jj-sc-fly{position:fixed;z-index:2147483400;pointer-events:none;width:34px;height:34px;object-fit:contain;transform:translate(-50%,-50%);filter:drop-shadow(0 4px 10px rgba(0,0,0,.35));}.jj-sc-fly.theme{width:46px;height:60px;}' +
    ...
    '#jj-ach.on{opacity:1;pointer-events:auto;}#jj-ach.closing{pointer-events:auto;}body.jj-modal-open .nav,body.jj-modal-open .nav *,body.jj-modal-open #jj-sc-hud,body.jj-modal-open #jj-sc-hud *,body.jj-modal-open .menu-container,body.jj-modal-open .nav-logo-link{pointer-events:none!important;}body.jj-modal-open .nav,body.jj-modal-open .nav *,body.jj-modal-open #jj-sc-hud,body.jj-modal-open .menu-container,body.jj-modal-open .nav-logo-link,body.jj-modal-open #jj-sound-btn,body.jj-modal-open #jj-sound-mist,body.jj-modal-open .audio-container-controller{z-index:0!important;}#jj-ach,#jj-first{z-index:10000;}' +
    'body.jj-modal-open .nav,body.jj-modal-open .nav *,body.jj-modal-open #jj-sc-hud,body.jj-modal-open #jj-sc-hud *,body.jj-modal-open .menu-container,body.jj-modal-open .nav-logo-link,body.jj-modal-open #jj-sound-btn,body.jj-modal-open #jj-sound-mist,body.jj-modal-open .audio-container-controller{pointer-events:none!important;}' +
    '.jjst-ov,#jjco-ask,.jj-overlay{-webkit-backdrop-filter:blur(8px)!important;backdrop-filter:blur(8px)!important;}' +   // every overlay blurs the page behind it   // RULE: while a modal is open the nav (logo, Themes, Store, score, Menu) sits BEHIND the overlay and cannot be clicked
    '#jj-ach .card{position:relative;width:min(620px,60vw);min-width:min(94vw,520px);height:min(800px,78vh);--sw:36px;border:var(--sw) solid transparent;display:flex;flex-direction:column;transform:translateY(16px) scale(.98);transition:transform .45s cubic-bezier(.22,1,.36,1);box-sizing:border-box;}' +   // the same band in every theme → nothing moves when the theme changes
    ...
    '#jj-ach .head{display:flex;align-items:center;justify-content:space-between;gap:16px;height:70px;padding:6px 10px 0 14px;flex:0 0 auto;box-sizing:border-box;}' +
    '#jj-ach h2{margin:0;font-size:clamp(20px,2.2vw,34px);font-weight:700;letter-spacing:.02em;min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}' +
    '#jj-ach .hr{display:flex;align-items:center;gap:8px;position:relative;flex:0 0 auto;margin-left:auto;}' +
    '.pinkw,#jj-sc-hud .jn>.pinkw,#jj-ach .pill>.pinkw,#jj-ach .theme>.pinkw,#jj-ach .x>.pinkw,#jj-ach .st>.pinkw,.jj-sc-glass>.pinkw{position:absolute!important;inset:0;overflow:hidden;border-radius:24.5px;pointer-events:none;z-index:0!important;}#jj-ach .x>.pinkw,#jj-ach .st>.pinkw{border-radius:50%;}.jj-sc-glass>.pinkw{border-radius:inherit;}html[data-jj-theme="medieval"] .pinkw,html[data-jj-theme="retro"] .pinkw{border-radius:4px;}#jj-ach.t-mixed .pinkw,html[data-jj-theme="mixed"] #jj-sc-hud .pinkw{border-radius:999px;}' +   // the press burst stays inside its button
    '#jj-ach h2{flex:0 1 auto;}#jj-ach .head .cnt{flex:1 1 auto;font-size:14px;font-weight:700;letter-spacing:.06em;opacity:.75;margin-left:10px;white-space:nowrap;}#jj-ach .row .rw .plus{opacity:.6;margin:0 2px;}' +   // 4/22 next to the title
    '#jj-ach .tabs.scr{-webkit-mask-image:linear-gradient(to right,#000 calc(100% - 64px),transparent);mask-image:linear-gradient(to right,#000 calc(100% - 64px),transparent);}' +   // more tabs off to the right
    '#jj-ach .row .ico{overflow:hidden;}#jj-ach .row[data-id^="credits"] .ico img{width:76%;height:76%;}' +   // the pills pin to the right edge in every theme; a long title truncates instead
    ...
    '#jj-ach .pill::before,#jj-ach .theme::before,#jj-ach .x::before,#jj-ach .st::before{content:"";position:absolute;inset:0;z-index:0;border-radius:24.5px;background:rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.5);-webkit-backdrop-filter:blur(20px);backdrop-filter:blur(20px);box-sizing:border-box;transition:background .2s ease,filter .2s ease;}#jj-ach .x::before,#jj-ach .st::before{border-radius:50%;}' +
    '#jj-ach .pill{min-width:132px;padding:0 12px;gap:6px;justify-content:center;}#jj-ach .pill .n{min-width:1ch;text-align:right;}#jj-ach .pill img{width:24px;height:22px;object-fit:contain;margin:0;}#jj-ach .theme{width:auto;padding:0 14px;gap:8px;cursor:pointer;font-weight:700;justify-content:flex-start;flex:0 0 auto;}#jj-ach .theme:hover::before{background:rgba(0,0,0,.92);}#jj-ach .theme svg{width:12px;height:7px;flex:0 0 auto;transition:transform .3s ease;}#jj-ach .theme.open svg{transform:rotate(180deg);}' +
    '.jj-tmenu{position:absolute;min-width:270px;font-family:' + FONT + ';text-align:left;background:rgba(0,0,0,.6);-webkit-backdrop-filter:blur(20px);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.5);border-radius:14px;padding:6px;z-index:6;box-shadow:0 16px 44px rgba(0,0,0,.55);opacity:0;pointer-events:none;transform:translateY(-6px);transition:opacity .2s ease,transform .25s ease;}.jj-tmenu.on{opacity:1;pointer-events:auto;transform:none;}#jj-ach .jj-tmenu{top:57px;left:0;}' +
    ...
    '#jj-ach .x{width:49px;cursor:pointer;padding:0;justify-content:center;transition:transform .25s ease;flex:0 0 auto;}#jj-ach .x svg{width:16px;height:16px;display:block;}#jj-ach .x:hover{transform:rotate(90deg);}#jj-ach .x:hover::before{background:rgba(0,0,0,.92);}' +
    '#jj-ach .tabs{display:flex;align-items:center;gap:5px;height:78px;padding:8px 14px;margin:0 14px;box-sizing:border-box;flex:0 0 auto;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;}#jj-ach .tabs::-webkit-scrollbar{display:none;}' +
    ...
    '#jj-ach .tab span{position:relative;}#jj-ach .tab.on .ic .on{opacity:1;}#jj-ach .tab.on .ic .off{opacity:0;}#jj-ach .tab .ndot{position:absolute;right:-7px;top:-7px;z-index:4;width:9px;height:9px;border-radius:50%;background:#FF00F5;box-shadow:0 0 0 2px rgba(8,14,35,.82),0 0 10px #FF00F5;opacity:0;transform:scale(0);pointer-events:none;}#jj-ach .tab.new .ndot{opacity:1;transform:none;animation:jjTabPing 1.25s ease-in-out infinite;}@keyframes jjTabPing{0%,100%{opacity:.55;scale:.82;}50%{opacity:1;scale:1.18;}}' +
    ...
    /* rows */
    '#jj-ach .row{position:relative;display:grid;grid-template-columns:auto 1fr;gap:0 18px;align-items:center;padding:16px 18px;height:auto;min-height:138px;flex:0 0 auto;border-radius:16px;overflow:visible;box-sizing:border-box;transition:box-shadow .3s ease,border-color .3s ease;}' +
    '#jj-ach .row .ico{width:72px;height:72px;border-radius:50%;border:2.5px solid;display:flex;align-items:center;justify-content:center;flex:0 0 auto;position:relative;z-index:1;box-sizing:border-box;}#jj-ach .row .ico img{width:88%;height:88%;object-fit:contain;}' +
    ...
    '#jj-ach .row .pr{display:flex;align-items:center;gap:10px;margin-top:12px;flex-wrap:wrap;width:100%;}#jj-ach .row .segs{display:flex;gap:6px;flex:1 1 200px;max-width:380px;}' +
    ...
    '#jj-ach .row .wmw{position:absolute;inset:0;overflow:hidden;border-radius:inherit;pointer-events:none;z-index:0;}#jj-ach .row .wm{position:absolute;right:-26px;bottom:-34px;transform:rotate(-10deg);width:190px;height:190px;object-fit:contain;opacity:.1;filter:grayscale(1) brightness(1.4);}#jj-ach .row[data-id="alien"] .ico img{transform:translateX(-8%);}#jj-ach .row[data-id="alien"] .wm{transform:translateX(-8%) rotate(-10deg);}' +
    '#jj-ach .row .chk{position:relative;margin-left:auto;flex:0 0 auto;width:30px;height:30px;border-radius:50%;background:#FFC531;display:none;align-items:center;justify-content:center;z-index:1;box-shadow:0 2px 8px rgba(0,0,0,.35);}#jj-ach .row .chk svg{width:16px;height:16px;}#jj-ach .row.done .chk{display:flex;}' +
    ...
    '#jj-ach .row .rbg{display:none;position:absolute;inset:-9px;z-index:0;border-radius:4px;pointer-events:none;}#jj-ach.t-mixed .row.done .rbg{display:block;background:#f3e2ba;}#jj-ach.t-mixed .row.done{color:#3a2a12;}#jj-ach.t-mixed .row.done .ds{color:#6b5535;}#jj-ach.t-mixed .row.done .ico{border-color:rgba(90,60,20,.6);}' +
    '#jj-ach .row.lock .rw img{filter:grayscale(1) brightness(1.1);opacity:.55;}#jj-ach .row.lock .ico{filter:grayscale(1);box-shadow:none!important;}#jj-ach .row.lock .ico img{filter:grayscale(1);opacity:.7;}#jj-ach .row.done .ico{filter:none;}' +
    '#jj-sc-hud .jn:focus-visible,#jj-ach button:focus-visible,#jj-ach .tab:focus-visible,.jj-tmenu .ti:focus-visible,#jj-first button:focus-visible{outline:3px solid #C5E7FF;outline-offset:3px;}html[data-jj-theme="retro"] #jj-sc-hud .jn:focus-visible,#jj-ach.t-retro button:focus-visible,#jj-ach.t-retro .tab:focus-visible,html[data-jj-theme="retro"] .jj-tmenu .ti:focus-visible,#jj-first.t-retro button:focus-visible{outline-color:#FFD400;box-shadow:0 0 0 3px #0B1E5A;}' +
    ...
    '#jj-ach.t-classic .bgd{display:block;inset:-18px;border-radius:26px;border:1.5px solid rgba(255,255,255,.5);background:rgba(0,0,0,.26);-webkit-backdrop-filter:blur(26px);backdrop-filter:blur(26px);overflow:hidden;}#jj-ach.t-classic .bgd::after{content:"";position:absolute;inset:0;background:rgba(0,0,0,.55);transform:scaleY(0);transform-origin:bottom center;transition:transform .5s cubic-bezier(.4,0,.2,1);}#jj-ach.t-classic .card:hover .bgd::after{transform:scaleY(.999);}' +   // the classic rule: the click-to-begin glass, filling up while you hover
    '#jj-ach.t-classic .bgd.__old{display:block;inset:-18px;border-radius:26px;border:1.5px solid transparent;background:linear-gradient(160deg,rgba(12,20,40,.78),rgba(6,12,24,.86)) padding-box,linear-gradient(135deg,#3b7dff 0%,#7a5cff 40%,#b04cff 70%,#ff3ec8 100%) border-box;-webkit-backdrop-filter:blur(22px);backdrop-filter:blur(22px);box-shadow:0 0 60px rgba(90,120,255,.28),0 0 120px rgba(255,0,245,.12),inset 0 0 60px rgba(60,90,200,.12);}' +
    ...
    '#jj-ach.t-classic .tab{border:1px solid transparent;border-image:none;border-radius:999px;height:44px;padding:0 18px;color:#cfd6e6;margin:0 2px;transition:color .25s ease,background .25s ease,border-color .25s ease;}#jj-ach.t-classic .tab+.tab::before{display:none;}' +
    '#jj-ach.t-classic .tab .bb{display:none;}#jj-ach.t-classic .tab .ic .off{filter:grayscale(1) brightness(1.7);}' +
    '#jj-ach.t-classic .tab::after{display:none;}#jj-ach.t-classic .tab.on{color:#fff;background:rgba(0,0,0,.4);border-color:rgba(255,255,255,.5);}#jj-ach.t-classic .tab:hover{color:#fff;background:rgba(0,0,0,.4);border-color:rgba(255,255,255,.25);}' +
    ...
    /* Special's dressing: the 8-bit Trogdor top-left, the alien bottom-right above the pixel waves, the moon */
    '#jj-ach.t-mixed .trog{display:block;left:1vw;top:7vh;width:clamp(160px,17vw,280px);height:auto;image-rendering:pixelated;animation:jjScTrog 7s ease-in-out infinite;}' +
    '#jj-ach.t-mixed .deco.b{display:block;position:fixed;left:0;width:100vw;bottom:0;height:26vh;background:url(' + RETRO.waves + ') bottom center / 100% 100% no-repeat;image-rendering:pixelated;opacity:.9;z-index:0;}' +
    '#jj-ach.t-mixed .side.r{display:block;width:clamp(130px,15vw,220px);height:auto;right:3vw;bottom:8vh;z-index:1;filter:drop-shadow(0 10px 18px rgba(0,0,0,.45));transform:scaleX(-1);}' +
    '#jj-ach.t-mixed .moon{display:block;right:6vw;top:6vh;width:clamp(110px,14vw,220px);height:auto;opacity:.95;filter:drop-shadow(0 0 30px rgba(199,231,255,.55));}' +
    '#jj-ach.t-mixed .jj-sc-glass{border:7px dashed #a8a8a8;border-radius:999px;background:linear-gradient(160deg,#0e1a33,#070f1d);box-shadow:inset 0 0 0 2px #6d6d6d,0 0 0 2px #6d6d6d,0 0 24px rgba(255,197,49,.18);color:#ffe9b0;text-shadow:0 0 10px rgba(255,197,49,.6);padding:.9rem 2.2rem;overflow:hidden;transition:box-shadow .3s ease,transform .45s cubic-bezier(.22,1,.36,1),opacity .3s ease;}' +
    '#jj-ach.t-mixed .jj-sc-glass::before{content:"";position:absolute;inset:0;background:linear-gradient(115deg,transparent 30%,rgba(255,197,49,.28) 50%,transparent 70%);transform:translateX(-120%);animation:jjScShimmer 3.6s ease-in-out infinite;z-index:0;}@keyframes jjScShimmer{0%,55%{transform:translateX(-120%);}100%{transform:translateX(120%);}}' +
    '#jj-ach.t-mixed .jj-sc-glass .fill{display:none;}#jj-ach.t-mixed .jj-sc-glass:hover{background:linear-gradient(160deg,#16264a,#0a1428);box-shadow:inset 0 0 0 2px #ffc531,0 0 0 2px #ffc531,0 0 34px rgba(255,197,49,.55);}#jj-ach.t-mixed .jj-sc-glass > span::before{content:none;}' +
    ...
    '#jj-ach.t-alien .jj-sc-glass{border:2.5px solid rgba(120,220,255,.85);border-radius:22px;background:linear-gradient(160deg,rgba(10,16,52,.92),rgba(6,10,32,.95));box-shadow:0 0 26px rgba(79,227,255,.3),inset 0 0 40px rgba(60,120,255,.16);}#jj-ach.t-alien .jj-sc-glass .fill{background:rgba(0,0,0,.3);}#jj-ach.t-alien .jj-sc-glass span::before{content:none;}' +
    ...
    '#jj-ach.t-retro .deco.b{display:block;position:fixed;left:0;width:100vw;bottom:0;height:26vh;background:url(' + RETRO.waves + ') bottom center / 100% 100% no-repeat;image-rendering:pixelated;opacity:.95;}' +
    '#jj-ach.t-retro .trog{display:block;left:1vw;top:7vh;width:clamp(160px,17vw,280px);height:auto;image-rendering:pixelated;animation:jjScTrog 7s ease-in-out infinite;}#jj-ach .trog{transition:opacity .9s ease,transform .9s cubic-bezier(.22,1,.36,1);}#jj-ach .trog.pre{opacity:0;transform:scale(.35);}#jj-ach .trog.gone{opacity:0;transform:scale(.25) translateY(-60px);transition:opacity .85s ease,transform .9s cubic-bezier(.5,0,.8,1);}@keyframes jjScTrog{0%,100%{translate:0 0;}50%{translate:3vw -18px;}}' +
    '#jj-ach.t-retro .banner{display:block;--bwid:clamp(72px,9%,112px);top:calc(-1 * var(--sw) - 2px);image-rendering:pixelated;filter:drop-shadow(6px 6px 0 rgba(0,0,0,.35));}#jj-ach.t-retro .banner.l{right:calc(100% + 2px);}#jj-ach.t-retro .banner.r{left:calc(100% + 2px);}' +
    '#jj-ach.t-retro .theme::before,#jj-ach.t-retro .pill::before,#jj-ach.t-retro .x::before,#jj-ach.t-retro .st::before{border-radius:0;border:12px solid transparent;border-image:url(' + RETRO.pill + ') 16 fill / 12px / 0 round;image-rendering:pixelated;background:none;box-shadow:none;}#jj-ach.t-retro .x:hover{transform:none;}#jj-ach.t-retro .theme:hover::before,#jj-ach.t-retro .x:hover::before,#jj-ach.t-retro .st:hover::before{background:none;border-image-source:url(' + RETRO.pill + ');filter:brightness(1.18) drop-shadow(3px 3px 0 rgba(255,0,245,.8));}' +
    ...
    '#jj-ach.t-retro .tab.on{border-image-source:url(' + RETRO.cardW + ');color:#0b1e5a;}#jj-ach.t-retro .tab.on::before{content:"▶";color:#e0322b;font-size:10px;margin-right:2px;}#jj-ach.t-retro .tab:hover,#jj-ach.t-retro .tab.on:hover{box-shadow:5px 5px 0 #FF00F5;}' +
    '#jj-ach.t-retro .list{scrollbar-color:#eeeade #0b1e5a;}#jj-ach.t-retro .list::-webkit-scrollbar-track{background:#0b1e5a;border-radius:0;}#jj-ach.t-retro .list::-webkit-scrollbar-thumb{background:#eeeade;border-radius:0;border-color:#0b1e5a;}#jj-ach.t-retro .more{color:#fff;text-shadow:2px 2px 0 #000;}#jj-ach.t-retro .more:hover{color:#FFD400;filter:drop-shadow(2px 2px 0 #FF00F5);}#jj-ach.t-retro .more svg path{stroke:currentColor;}' +
    ...
    /* retro CTA: keep the cream pixel silhouette; hover adds light and a magenta pixel shadow */
    '#jj-ach.t-retro .jj-sc-glass{border:21px solid transparent;border-image:url(' + RETRO.pillW + ') 28 fill / 21px / 0 round;image-rendering:pixelated;border-radius:0;background:none;color:#0b1e5a;box-shadow:none;-webkit-backdrop-filter:none;backdrop-filter:none;font-size:.95rem;padding:.3rem 1.3rem;transition:filter .15s ease,transform .45s cubic-bezier(.22,1,.36,1),opacity .3s ease;}#jj-ach.t-retro .jj-sc-glass:hover{background:none;box-shadow:none;border-image-source:url(' + RETRO.pillW + ');filter:brightness(1.12) drop-shadow(4px 4px 0 #FF00F5);}#jj-ach.t-retro .jj-sc-glass .fill{display:none;}#jj-ach.t-retro .jj-sc-glass svg{display:none;}#jj-ach.t-retro .jj-sc-glass > span::before{content:"◀";color:#e0322b;font-size:13px;margin-right:14px;}#jj-ach.t-retro .jj-sc-glass:active{filter:brightness(.9) drop-shadow(2px 2px 0 #FF00F5);}' +
    ...
    '#jj-first.t-classic .card{border-image:none;border-radius:26px;border:1.5px solid rgba(255,255,255,.5);padding:40px 40px 28px;background:rgba(0,0,0,.26);-webkit-backdrop-filter:blur(26px);backdrop-filter:blur(26px);-webkit-backdrop-filter:blur(22px);backdrop-filter:blur(22px);box-shadow:0 0 60px rgba(90,120,255,.28);}' +
    ...
    '#jj-first.t-medieval .card{border-image:url(' + IMG.frame + ') 96 fill / 32px / 0 round;color:#3a2a12;padding:10px 26px 22px 30px;}#jj-first.t-medieval .card p{color:#5a4526;}' +
    ...
    '#jj-first.t-classic .jj-sc-btn,#jj-first.t-alien .jj-sc-btn,#jj-first.t-mixed .jj-sc-btn{border-image:none;border:2px solid rgba(255,255,255,.5);border-radius:8px;background:rgba(0,0,0,.4);}#jj-first.t-classic .jj-sc-btn .bb,#jj-first.t-alien .jj-sc-btn .bb,#jj-first.t-mixed .jj-sc-btn .bb{display:none;}#jj-first.t-classic .jj-sc-btn:hover,#jj-first.t-mixed .jj-sc-btn:hover{background:rgba(0,0,0,.92);}#jj-first.t-alien .jj-sc-btn{border-color:#4fe3ff;box-shadow:0 0 14px rgba(79,227,255,.3);}#jj-first.t-alien .jj-sc-btn:hover{background:rgba(79,227,255,.18);}' +
    '#jj-first.t-retro .jj-sc-btn{border:12px solid transparent;border-image:url(' + RETRO.pill + ') 16 fill / 12px / 0 round;image-rendering:pixelated;background:none;font-family:"Mario","Joes Journey Headline",sans-serif;font-size:12px;min-height:0;padding:4px 10px;}#jj-first.t-retro .jj-sc-btn .bb{display:none;}#jj-first.t-retro .jj-sc-btn:hover{border-image-source:url(' + RETRO.pill + ');transform:none;filter:brightness(1.18) drop-shadow(3px 3px 0 #FF00F5);}' +
    ...
    'html[data-jj-theme="classic"] .jjst-ov .card{border:1.5px solid rgba(255,255,255,.5);border-radius:26px;background:rgba(0,0,0,.26);-webkit-backdrop-filter:blur(26px);backdrop-filter:blur(26px);box-shadow:0 0 60px rgba(0,0,0,.5);}' +
    ...
    '#jj-ach .store{display:none;position:relative;z-index:1;flex:1 1 auto;min-height:0;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:10px 40px 30px;gap:8px;}#jj-ach.store .store{display:flex;}#jj-ach.store .tabs.atabs{display:none;}#jj-ach:not(.store) .tabs.stabs{display:none;}#jj-ach.store.noreg .lw,#jj-ach.store.noreg .tabs.stabs{display:none;}#jj-ach.store:not(.noreg) .store{display:none;}' +
    ...
    /* one geometry for every skin: themes may repaint it, never move it */
    '#jj-ach .card{width:min(840px,60vw);min-width:min(94vw,560px);}' +
    /* RULE: the panel never rises into the nav. The overlay is padded below the nav (plus room for the decorations on the frame) and the card shrinks to fit */
    '#jj-ach{padding:78px 0 12px;box-sizing:border-box;}#jj-ach .card{height:min(800px,78vh);max-height:calc(100vh - 78px - 12px - 78px);}' +   // clear of the nav band, but back up the screen
    '#jj-ach .tabs,#jj-ach.t-classic .tabs,#jj-ach.t-retro .tabs{height:78px;padding:8px 14px;margin:0 14px;gap:6px;border-bottom:1px solid rgba(255,255,255,.12);}' +
    '#jj-ach .list{padding:14px 10px 22px 14px;gap:12px;}' +
    '#jj-ach .row,#jj-ach.t-retro .row{height:auto;min-height:138px;padding:16px 18px;grid-template-columns:72px minmax(0,1fr);column-gap:18px;box-sizing:border-box;}' +
    '#jj-ach .row .ico,#jj-ach.t-retro .row .ico{width:72px;height:72px;box-sizing:border-box;}' +
    '#jj-ach.t-medieval .row.done,#jj-ach.t-mixed .row.done{border-width:1.5px;border-style:solid;border-image:none;border-radius:16px;padding:16px 18px;}' +
    '#jj-ach.t-medieval .row.done,#jj-ach.t-mixed .row.done{margin:14px 18px 20px;}#jj-ach.t-medieval .row.done .rbg,#jj-ach.t-mixed .row.done .rbg{display:block;inset:-15px;border:20px solid transparent;border-image:url(' + IMG.frame + ') 96 / 20px / 0 round;border-radius:0;box-sizing:border-box;}' +
    '#jj-ach.t-mixed .row.done .rbg{background:#f3e2ba;}' +
    '#jj-first .btns{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));width:min(100%,460px);margin-inline:auto;}#jj-first .btns .jj-sc-btn{width:100%;}' +
    'html[data-jj-theme] .jjst-ov .row{display:flex!important;flex-wrap:wrap;justify-content:center;gap:14px;width:min(100%,620px);margin-inline:auto;}html[data-jj-theme] .jjst-ov .row .jjst-glass{width:auto;min-width:0;padding:.85rem 1.5rem!important;font-size:1rem!important;}' +   // the prompt buttons hug their labels
    '@media(max-width:620px){#jj-ach .card{width:94vw;}#jj-first .btns,html[data-jj-theme] .jjst-ov .row{grid-template-columns:1fr;}#jj-ach .row,#jj-ach.t-retro .row{padding-right:54px;}#jj-ach .row .chk{right:12px;}}' +

    'html[data-jj-theme="classic"] .audio-container-controller{border:1px solid rgba(255,255,255,.5)!important;background:rgba(0,0,0,.4)!important;-webkit-backdrop-filter:blur(20px);backdrop-filter:blur(20px);}' +   // the sound bar joins the classic rule
    /* Special: the stone ring from Back to Exploring on every button */
    'html[data-jj-theme="mixed"] #jj-sc-hud .jn::before,html[data-jj-theme="mixed"] #jjst-hint::before,html[data-jj-theme="mixed"] #jjst-ctl .jb::before,html[data-jj-theme="mixed"] #jjms-next::before,#jj-ach.t-mixed .pill::before,#jj-ach.t-mixed .theme::before,#jj-ach.t-mixed .x::before,#jj-ach.t-mixed .st::before{border-radius:999px!important;border:4px dashed #a8a8a8!important;border-image:none!important;background:linear-gradient(160deg,#0e1a33,#070f1d)!important;box-shadow:inset 0 0 0 2px #6d6d6d,0 0 0 2px #6d6d6d,0 0 16px rgba(255,197,49,.14)!important;backdrop-filter:none!important;}' +
    'html[data-jj-theme="mixed"] #jj-sc-hud .jn,#jj-ach.t-mixed .pill,#jj-ach.t-mixed .theme,#jj-ach.t-mixed .x,#jj-ach.t-mixed .st{color:#ffe9b0;text-shadow:0 0 10px rgba(255,197,49,.5);}html[data-jj-theme="mixed"] #jj-sc-hud .jn:hover::before,html[data-jj-theme="mixed"] #jjst-hint:hover::before,html[data-jj-theme="mixed"] #jjst-ctl .jb:hover::before,html[data-jj-theme="mixed"] #jjms-next:hover::before,#jj-ach.t-mixed .theme:hover::before,#jj-ach.t-mixed .x:hover::before,#jj-ach.t-mixed .st:hover::before{background:linear-gradient(160deg,#16264a,#0a1428)!important;box-shadow:inset 0 0 0 2px #ffc531,0 0 0 2px #ffc531,0 0 24px rgba(255,197,49,.5)!important;}' +
    '#jj-ach.t-mixed .jjco-btn,#jj-first.t-mixed .jj-sc-btn,html[data-jj-theme="mixed"] .jjst-ov .jjst-glass{border:4px dashed #a8a8a8!important;border-image:none!important;border-radius:999px!important;background:linear-gradient(160deg,#0e1a33,#070f1d)!important;box-shadow:inset 0 0 0 2px #6d6d6d,0 0 0 2px #6d6d6d;color:#ffe9b0!important;text-shadow:0 0 10px rgba(255,197,49,.5);}#jj-ach.t-mixed .jjco-btn:hover,#jj-first.t-mixed .jj-sc-btn:hover,html[data-jj-theme="mixed"] .jjst-ov .jjst-glass:hover{background:linear-gradient(160deg,#16264a,#0a1428)!important;box-shadow:inset 0 0 0 2px #ffc531,0 0 0 2px #ffc531,0 0 24px rgba(255,197,49,.5);}#jj-ach.t-mixed .jjco-btn.buy{background:linear-gradient(160deg,#3a0c3a,#1a0620)!important;}' +
    '.jj-sc-conf{position:fixed;top:-20px;z-index:2147483300;pointer-events:none;animation:jjScConf linear forwards;}@keyframes jjScConf{0%{transform:translateY(0) rotate(0deg);opacity:1;}100%{transform:translateY(110vh) rotate(720deg);opacity:.9;}}' +
    '#jj-first.secret .card{--sw:32px;}#jj-first.secret h3{font-size:clamp(28px,4vw,44px);}#jj-first.secret .big{width:min(60vw,260px);height:auto;}' +
  ...
  function menuKey(e) { if ((e.key === 'Enter' || e.key === ' ') && e.target && e.target.classList.contains('ti')) { e.preventDefault(); e.target.click(); } }
  function mount() {
    ...
    var storePeek = document.createElement('img'); storePeek.className = 'store-peek'; storePeek.src = GB + 'ach-alien-catch.webp'; storePeek.alt = ''; storePeek.setAttribute('aria-hidden', 'true'); storePeek.addEventListener('error', function () { storePeek.hidden = true; }); hud.querySelector('.jn-st').prepend(storePeek);
    ...
    thB.addEventListener('click', function (e) { e.stopPropagation(); var wait = 0; setTimeout(function () { if (navOpen === thM) closeNav(); else { closeNav(); thM.innerHTML = menuHTML(); bindMenu(thM, closeNav, true); thM.classList.add('on'); thB.classList.add('open'); navOpen = thM; } }, wait); });   // from inside the main menu: the menu goes first, then the dropdown
    scB.addEventListener('click', function (e) { e.stopPropagation(); var wait = menuAway() ? 320 : 0; setTimeout(function () { if (navOpen === scM) closeNav(); else { closeNav(); var u = unseen();
      scM.innerHTML = '<div class="ti" role="button" tabindex="0" data-go="ach" data-cursor="hover"><img class="mi" data-ico="star" src="' + ico('star') + '" alt=""><span class="tx">Achievements' + (u ? '<span class="sub">' + u + ' new</span>' : '') + '</span></div><div class="ti" role="button" tabindex="0" data-go="store" data-cursor="hover"><svg class="bk" viewBox="167 15 20 19" fill="currentColor"><path d="M170.024 33.5C169.474 33.5 169.003 33.3042 168.611 32.9125C168.219 32.5208 168.024 32.05 168.024 31.5V23.55C167.64 23.2 167.344 22.75 167.136 22.2C166.928 21.65 166.924 21.05 167.124 20.4L168.174 17C168.307 16.5667 168.544 16.2083 168.886 15.925C169.228 15.6417 169.624 15.5 170.074 15.5H183.974C184.424 15.5 184.815 15.6375 185.149 15.9125C185.482 16.1875 185.724 16.55 185.874 17L186.924 20.4C187.124 21.05 187.119 21.6417 186.911 22.175C186.703 22.7083 186.407 23.1667 186.024 23.55V31.5C186.024 32.05 185.828 32.5208 185.436 32.9125C185.044 33.3042 184.574 33.5 184.024 33.5H170.024ZM179.224 22.5C179.674 22.5 180.015 22.3458 180.249 22.0375C180.482 21.7292 180.574 21.3833 180.524 21L179.974 17.5H178.024V21.2C178.024 21.55 178.14 21.8542 178.374 22.1125C178.607 22.3708 178.89 22.5 179.224 22.5ZM174.724 22.5C175.107 22.5 175.419 22.3708 175.661 22.1125C175.903 21.8542 176.024 21.55 176.024 21.2V17.5H174.074L173.524 21C173.457 21.4 173.544 21.75 173.786 22.05C174.028 22.35 174.34 22.5 174.724 22.5ZM170.274 22.5C170.574 22.5 170.836 22.3917 171.061 22.175C171.286 21.9583 171.424 21.6833 171.474 21.35L172.024 17.5H170.074L169.074 20.85C168.974 21.1833 169.028 21.5417 169.236 21.925C169.444 22.3083 169.79 22.5 170.274 22.5ZM183.774 22.5C184.257 22.5 184.607 22.3083 184.824 21.925C185.04 21.5417 185.09 21.1833 184.974 20.85L183.924 17.5H182.024L182.574 21.35C182.624 21.6833 182.761 21.9583 182.986 22.175C183.211 22.3917 183.474 22.5 183.774 22.5ZM170.024 31.5H184.024V24.45C183.94 24.4833 183.886 24.5 183.861 24.5H183.774C183.324 24.5 182.928 24.425 182.586 24.275C182.244 24.125 181.907 23.8833 181.574 23.55C181.274 23.85 180.932 24.0833 180.549 24.25C180.165 24.4167 179.757 24.5 179.324 24.5C178.874 24.5 178.453 24.4167 178.061 24.25C177.669 24.0833 177.324 23.85 177.024 23.55C176.74 23.85 176.411 24.0833 176.036 24.25C175.661 24.4167 175.257 24.5 174.824 24.5C174.34 24.5 173.903 24.4167 173.511 24.25C173.119 24.0833 172.774 23.85 172.474 23.55C172.124 23.9 171.778 24.1458 171.436 24.2875C171.094 24.4292 170.707 24.5 170.274 24.5H170.161C170.119 24.5 170.074 24.4833 170.024 24.45V31.5Z"/></svg><span class="tx">Store</span></div>';
      ...
      scM.classList.add('on'); scB.classList.add('open'); navOpen = scM; } }, wait); });
    hud.querySelector('.jn-st').addEventListener('click', function (e) { e.stopPropagation(); closeNav(); openPanel('store'); });
    hud.querySelectorAll('.jn').forEach(pinkPress);
    document.addEventListener('click', function (e) { if (navOpen && !(e.target.closest && e.target.closest('#jj-sc-hud'))) closeNav(); }, true);
    hud.addEventListener('keydown', menuKey);
    place();
    setTimeout(checkMeta, 2500);                            // combination rows (Part Two, A Bit of Everything) catch up on parts earned before they existed
    /* the daily bonus: +10 coins on any visit on a later day than the first, once a day */
    var dn = new Date(), today = dn.getFullYear() + '-' + (dn.getMonth() + 1) + '-' + dn.getDate();
    if (!S.first) { S.first = today; save(); } else if (S.first !== today && S.daily !== today) { S.daily = today; save(); setTimeout(function () { credit(10, 'Daily bonus'); }, 4500); }
    /* the three case studies: reading each one is a part of Case Closed */
    var cs = (location.pathname.match(/^\/case-studies\/([a-z0-9-]+)/) || [])[1]; if (cs) setTimeout(function () { award('casestudies', { part: cs }); }, 1800);
  ...
  function countTo(el, from, to, ms) {                     // 1, 2, 3 … really quickly
    var dir = to >= from ? 1 : -1, steps = Math.max(1, Math.abs(to - from)), i = 0, t = Math.max(18, Math.min(60, ms / steps));
    el.classList.add('up');
    (function tick() { i++; var v = from + dir * i; el.textContent = dir > 0 ? Math.min(to, v) : Math.max(to, v); if (i < steps) setTimeout(tick, t); else setTimeout(function () { el.classList.remove('up'); }, 220); })();
  }
  function land(kind) {
    if (!hud) return; sfx(kind === 'star' ? 'star' : kind === 'theme' ? 'theme' : 'coin');
  ...
  function payout(a, o) {
    ...
    floatAt(x, y - 18, (kind === 'star' ? '+' + (a.stars || 1) : kind === 'coin' ? '+' + (a.reward || COIN) : (a.star ? '+1 <img src="' + ico('star') + '" alt=""> · ' : '') + 'Theme unlocked!') + ' <img src="' + ico(kind) + '" alt="">');
    if (kind === 'theme' && a.star) setTimeout(function () { fly('star', x, y, function () { land('star'); }); }, 250);
    if (a.stars > 1) for (var k = 1; k < a.stars; k++) (function (k) { setTimeout(function () { fly('star', x + (k - 2) * 26, y, function () { land('star'); }); }, k * 140); })(k);   // five stars, a little fan of them
    fly(kind, x, y, function () {
      ...
      if (a.secret) { setTimeout(function () { openFirst('secret', a); confetti(); }, 350); }
      else if (kind === 'theme') { setTimeout(function () { openFirst('theme', a); }, 350); }
  ...
  function pinkPress(el) {                                   // the pink burst from click to begin, on any button
    if (!el || el._pp) return; el._pp = true;
    el.addEventListener('pointerdown', function (e) { var r = el.getBoundingClientRect(), x = e.clientX - r.left, y = e.clientY - r.top;
      ...
      var w = el.querySelector(':scope > .pinkw'); if (!w) { w = document.createElement('i'); w.className = 'pinkw'; el.insertBefore(w, el.firstChild); }
      var c = document.createElement('i'); c.className = 'pink'; c.style.cssText = 'position:absolute;border-radius:50%;background:#FF00F5;opacity:.9;pointer-events:none;transform:scale(0);transition:transform .4s ease-out,opacity .4s ease;width:' + d + 'px;height:' + d + 'px;left:' + (x - d / 2) + 'px;top:' + (y - d / 2) + 'px;'; w.appendChild(c);
  ...
  function buildPanel() {
    ...
    panel.innerHTML = '<div class="card"><i class="bgd"></i><img class="banner l" src="' + IMG.bannerL + '" alt=""><img class="banner r" src="' + IMG.bannerR + '" alt=""><img class="peek" src="' + ALIEN_PEEK + '" alt="">' +
      '<div class="head"><h2>Achievements</h2><span class="cnt" title="Collected"></span><div class="hr">' +
      ...
      '<div class="tabs atabs">' + TABS.map(function (t) { var ic = GB + 'score-tab-' + TABICON[t[0]]; return '<div class="tab" role="tab" tabindex="0" data-tab="' + t[0] + '" data-cursor="hover"><i class="bb"></i><i class="ic"><img class="off" src="' + ic + '-off.webp" alt="" onerror="this.style.display=\'none\'"><img class="on" src="' + ic + '-on.webp" alt="" onerror="this.style.display=\'none\'"></i><span>' + t[1] + '</span><i class="ndot" aria-hidden="true"></i></div>'; }).join('') + '</div><div class="tabs stabs"></div>' +
    ...
    panel.querySelectorAll('.card > .deco, .card > .trog, .card > .side, .card > .moon, .card > .sw').forEach(function (el) { panel.appendChild(el); });   // the dressing sits on the panel itself: behind the card, full viewport
    panel.addEventListener('keydown', menuKey);
    (function () { var tv = panel.querySelector('.trog'); if (!tv) return; tv.loop = false; tv.removeAttribute('loop'); var busy = false;
      var again = function () { tv.classList.add('pre'); tv.classList.remove('gone'); try { tv.currentTime = 0; } catch (e) {} var pp = tv.play(); if (pp && pp.catch) pp.catch(function () {}); setTimeout(function () { tv.classList.remove('pre'); busy = false; }, 60); };
      tv.addEventListener('timeupdate', function () { if (!busy && tv.duration && tv.currentTime > tv.duration - .75) { busy = true; tv.classList.add('gone'); } });   // off into the distance just before the clip ends
      tv.addEventListener('ended', function () { setTimeout(again, 1600); });
    })();
    ...
    panel.querySelector('.st').addEventListener('click', function (e) { e.stopPropagation(); setMode(panel.classList.contains('store') ? 'ach' : 'store'); renderRows(); });
    buildStoreTabs();
    ...
    panel.querySelectorAll('.atabs .tab').forEach(function (b) { var go = function (e) { e.preventDefault(); e.stopPropagation(); curTab = b.getAttribute('data-tab'); seeTab(curTab); renderRows(); };
    ...
    var back = panel.querySelector('#jj-ach-back'); back.addEventListener('click', closePanel); back.addEventListener('mouseenter', function () { flick(back.querySelector('span')); });
    ['.theme', '.x', '.st'].forEach(function (q) { pinkPress(panel.querySelector(q)); });
    pinkPress(back);
    if (false) back.addEventListener('pointerdown', function (e) { var r = back.getBoundingClientRect(), x = e.clientX - r.left, y = e.clientY - r.top;   // the pink press, as on click to begin
      var d = Math.max(Math.hypot(x, y), Math.hypot(r.width - x, y), Math.hypot(x, r.height - y), Math.hypot(r.width - x, r.height - y)) * 2.4;
  ...
  function menuHTML() {
    return THEMES.map(function (t) { var ok = themeOk(t.id), a = t.by && BY[t.by];
      return '<div class="ti' + (S.theme === t.id ? ' on' : '') + (ok ? '' : ' lock') + '" role="button" tabindex="0" data-theme="' + t.id + '" data-cursor="hover">' + (ok ? TICK : LOCK) + '<span class="tx">' + t.name + (ok ? '' : '<span class="sub">Locked · ' + (a ? a.name : '') + ' →</span>') + '</span></div>'; }).join('');
  }
  function bindMenu(el, close, quiet) {                     // the same list in the panel header and the nav's Themes pill; quiet = just switch, stay where you are
    el.querySelectorAll('.ti').forEach(function (it) { it.addEventListener('click', function (e) { e.stopPropagation(); var id = it.getAttribute('data-theme'), t = THEMES.filter(function (x) { return x.id === id; })[0];
      if (themeUse(id)) { S.theme = id; save(); themeAttr(); if (panel) { applyTheme(); renderRows(); } close(); if (!themeOk(id) && !quiet) hint(t.by); }   // in the panel a locked pick also shows the achievement that unlocks it
      else if (quiet) { close(); } else { close(); hint(t.by); } }); });
  }
  ...
  var storeReg = null, curStoreTab = null;
  function buildStoreTabs() { if (!panel) return; var st = panel.querySelector('.stabs'); if (!st) return; st.innerHTML = storeReg ? storeReg.tabs.map(function (t) { return '<div class="tab" role="tab" tabindex="0" data-stab="' + t[0] + '" data-cursor="hover"><i class="bb"></i><span>' + t[1] + '</span></div>'; }).join('') : '';
    st.querySelectorAll('.tab').forEach(function (b) { var go = function (e) { e.preventDefault(); e.stopPropagation(); curStoreTab = b.getAttribute('data-stab'); renderRows(); }; b.addEventListener('click', go); b.addEventListener('keydown', function (e) { if (e.key === 'Enter' || e.key === ' ') go(e); }); }); }
  function tabsFade() { if (!panel) return; panel.querySelectorAll('.tabs').forEach(function (t) { var f = function () { t.classList.toggle('scr', t.scrollWidth > t.clientWidth + 4 && t.scrollLeft + t.clientWidth < t.scrollWidth - 4); }; if (!t._fd) { t._fd = true; t.addEventListener('scroll', f, { passive: true }); window.addEventListener('resize', f); } f(); }); }
  function inStore() { return !!(panel && panel.classList.contains('store') && storeReg); }
  function counts() { if (!panel) return; var c = panel.querySelector('.head .cnt'); if (!c) return; var st = panel.classList.contains('store'), n, t;
    if (st) { var k = storeReg && storeReg.count ? storeReg.count() : null; n = k ? k.own : 0; t = k ? k.total : 0; } else { n = ACH.filter(function (a) { return S.done[a.id]; }).length; t = ACH.length; }
    c.textContent = n + '/' + t; c.setAttribute('aria-label', n + ' of ' + t + (st ? ' owned' : ' collected')); }
  function setMode(m) { panel.classList.toggle('store', m === 'store'); panel.classList.toggle('noreg', !storeReg); panel.querySelector('h2').textContent = m === 'store' ? 'Store' : 'Achievements'; panel.querySelector('.st').setAttribute('aria-label', m === 'store' ? 'Achievements' : 'Store'); }
  function hint(id) {                                         // take the visitor to the achievement that unlocks it, outline it, give it a pulse
    var a = BY[id]; if (!a) return; if (panel && panel.classList.contains('store')) setMode('ach'); curTab = a.tab; renderRows();
  ...
  function renderRows() {
    if (!panel) return;
    totEl.querySelector('.ts').textContent = stars(); totEl.querySelector('.tc').textContent = coins(); counts(); tabsFade();
    if (inStore()) {                                          // the store's own tabs + list come from jj-companion
      var tabs = storeReg.tabs; if (!tabs.some(function (t) { return t[0] === curStoreTab; })) curStoreTab = tabs[0][0];
      panel.querySelectorAll('.stabs .tab').forEach(function (b) { var on = b.getAttribute('data-stab') === curStoreTab; b.classList.toggle('on', on); b.setAttribute('aria-selected', on ? 'true' : 'false'); });
      listEl.innerHTML = ''; try { storeReg.render(curStoreTab, listEl); } catch (e) { listEl.innerHTML = '<div class="empty">The store is having a moment.</div>'; }
      listEl.scrollTop = 0; if (panel._edges) setTimeout(panel._edges, 30); return;
    }
    panel.querySelectorAll('.tab').forEach(function (b) { var id = b.getAttribute('data-tab'), on = id === curTab; b.classList.toggle('on', on); b.classList.toggle('new', tabNew(id)); b.setAttribute('aria-selected', on ? 'true' : 'false'); });
    ...
    listEl.innerHTML = rows.length ? rows.map(function (a) {
      ...
      var nm = d && a.doneName ? a.doneName : a.name, ds = d && a.doneDesc ? a.doneDesc : a.desc;
      var rw = a.kind === 'theme' ? (a.star ? '<img src="' + ico('star') + '" alt=""><span>1</span> <span class="plus">+</span> ' : '') + '<img class="th" src="' + IMG.theme + '" alt=""><span>Theme</span>' : '<img src="' + ico(a.kind) + '" alt=""><span>' + (a.kind === 'star' ? (a.stars || 1) : (a.reward || COIN)) + '</span>';
      var im = '<img src="' + ic.src + '" alt="" onerror="if(this.src!==this.dataset.fb){this.src=this.dataset.fb;}" data-fb="' + ic.fb + '">';
      return '<div class="row ' + (d ? 'done' : 'lock') + (d && !S.seenAch[a.id] ? ' fresh' : '') + '" data-id="' + a.id + '"><i class="rbg"></i>' +
        '<div class="ico">' + im + '</div><div class="mid"><div class="nm">' + nm + '</div><div class="ds">' + ds + '</div>' +
        '<div class="pr"><div class="segs">' + segs + '</div><span class="cnt">' + p + '/' + a.target + '</span><span class="rw">' + rw + '</span><i class="chk"><svg viewBox="0 0 16 16" fill="none"><path d="M3 8.5l3.2 3L13 4.5" stroke="#3a2a12" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg></i></div></div>' +
        '<i class="wmw">' + im.replace('<img ', '<img class="wm" ') + '</i></div>';
    }).join('') : '<div class="empty">Nothing here yet.</div>';
    listEl.scrollTop = 0; if (panel._edges) setTimeout(panel._edges, 30);
  }
  function menuAway() { if (document.body.classList.contains('jj-menu-open')) { var b = document.querySelector('.menu-container'); if (b) b.click(); return true; } return false; }
  function openPanel(mode) { if (menuAway()) { setTimeout(function () { openPanelNow(mode); }, 430); return; } openPanelNow(mode); }   // let the main menu finish closing first — pause() would freeze its animation half way
  function openPanelNow(mode) { buildPanel(); if (mode) setMode(mode); else if (!panelOpen && panel.classList.contains('store')) setMode('ach'); panelOpen = true; if (mode !== 'store') seeTab(curTab); applyTheme(); renderRows(); panel.querySelectorAll('video').forEach(function (tv) { try { tv.addEventListener('playing', function () { tv.removeAttribute('poster'); }, { once: true }); var tp = tv.play(); if (tp && tp.catch) tp.catch(function () {}); } catch (e) {} }); if (hud) hud.classList.remove('has-new'); clearTimeout(closingT); panel.classList.remove('closing'); panel.classList.add('on'); modal(); pause(); sfx('open'); }
  ...
  function openFirst(kind, a, go) {
    if (!first) {
      ...
      first.querySelector('.go').addEventListener('click', function () { closeFirst(true); if (first._go) { var g = first._go; first._go = null; g(); return; } openPanel(); if (first._theme) setTimeout(openMenu, 500); });
    ...
    first._theme = kind === 'theme'; first._go = go || null;
    first.querySelector('.big').src = c.img || ico(kind === 'custom' ? 'coin' : kind); first.querySelector('h3').textContent = c.title; first.querySelector('p').textContent = c.body.replace('{name}', th ? th.name : 'new');
    first.querySelector('.go span').textContent = kind === 'theme' ? 'TRY IT ON' : kind === 'custom' ? 'CUSTOMISE' : 'VIEW ACHIEVEMENTS'; first.classList.toggle('secret', kind === 'secret');
    menuAway(); first.className = 't-' + (themeUse(S.theme) ? S.theme : 'classic'); firstOpen = true; modal(); pause(); first.classList.add('on');
  }
  function confetti() {                                      // a shower of stars + pink and gold bits for the secret
    var n = 70, cols = ['#FF00F5', '#FFC531', '#fff', '#4fe3ff', '#b04cff'];
    for (var i = 0; i < n; i++) (function (i) { var p = document.createElement('i'); p.className = 'jj-sc-conf'; var x = Math.random() * 100, d = 2.2 + Math.random() * 1.6, dl = Math.random() * .9;
      p.style.cssText = 'left:' + x + 'vw;background:' + cols[i % cols.length] + ';animation-duration:' + d + 's;animation-delay:' + dl + 's;transform:rotate(' + (Math.random() * 360) + 'deg);width:' + (6 + Math.random() * 8) + 'px;height:' + (6 + Math.random() * 10) + 'px;border-radius:' + (Math.random() > .5 ? '50%' : '2px');
      document.body.appendChild(p); setTimeout(function () { p.remove(); }, (d + dl) * 1000 + 100); })(i);
  }
  ...
  window.jjScore = { award: award, has: has, progress: progress, stars: stars, coins: coins, earnedStars: earnedStars, earnedCoins: earnedCoins, spend: spend, credit: credit, open: function () { openPanel('ach'); }, store: function () { openPanel('store'); }, close: closePanel, reset: reset, ACH: ACH, THEMES: THEMES, BY: BY, GB: GB, ico: ico, themeOk: themeOk,
    registerStore: function (reg) { storeReg = reg; buildStoreTabs(); if (panel && panel.classList.contains('store')) { panel.classList.remove('noreg'); if (panelOpen) renderRows(); } },
    setStoreTab: function (id) { buildPanel(); curStoreTab = id; if (!panelOpen) openPanel('store'); else { if (!panel.classList.contains('store')) setMode('store'); renderRows(); } },
    goto: function (id) { openPanel('ach'); setTimeout(function () { hint(id); }, 60); },
    card: function (kind, go) { openFirst(kind, null, go); },
    music: function (id) { if (id) setMusic(id); return S.music || 'classic'; }, MUSIC: MUSIC, musicOk: musicOk,
  ...
  if (S.music && S.music !== 'classic') applyMusic();
})();
