/* jj-score.js — site-wide stars + coins + achievements (build k24: final score map, Level 6 mixed step and exact row grid).
...
(function () {
  ...
  var ACH = [
    { id: 'scroll',        tab: 'general',   kind: 'theme', theme: 'alien',    name: 'Journeyman',  desc: 'Make it to the end of the journey to unlock the Space theme' },
    { id: 'scroll-star',   tab: 'general',   kind: 'star',  name: 'End of the Line', desc: 'Finish the horizontal journey' },
    ...
    { id: 'storytime-star',tab: 'storytime', kind: 'star',  name: 'Once Upon a Time', desc: 'Finish the animated Tale of Trogdor' },
    ...
    { id: 'credits-star',  tab: 'credits',   kind: 'star',  name: 'Final Curtain',    desc: 'Finish the credits' },
  ...
  function coins() { return ACH.filter(function (a) { return a.kind === 'coin' && S.done[a.id]; }).reduce(function (n, a) { return n + (a.reward || COIN); }, 0); }
  ...
  var ACH_ART = {
    scroll: 'joe-evo-7.webp', 'scroll-star': 'joe-evo-7.webp', 'alien-catch': 'ach-alien-catch.png', quiz80: 'ach-quiz80.png', allrounder: 'ach-allrounder.png', planets: 'ach-planets.png',
    alien: 'alien-sit-idle-poster.webp', quiz: 'ach-quiz.png', quizFull: 'ach-quizfull.png', storytime: 'ach-storytime.png', bones: 'ach-bones.png',
    'big-bangs': 'ach-allrounder.png', 'sound-on': 'ach-alien.png', 'storytime-star': 'ach-storytime.png', 'story-secrets': 'ach-storytime.png',
    invasion: 'ach-invasion.png', trogdor: 'ach-trogdor.png', treasure: 'ach-treasure.webp', linkedin: 'icon-linkedin-fill.png', contact: 'icon-mail-fill.png', cv: 'icon-cv-fill.png',
    'credits-click': 'icon-credits-fill.png', credits5: 'levelup-5.svg', credits: 'end-screen.svg', 'credits-star': 'end-screen.svg', credits10: 'levelup-8.svg'
  };
  ...
  var css = document.createElement('style'); css.textContent =
    ...
    '#jj-sc-hud .jn:hover::before{background:rgba(0,0,0,.72);}#jj-sc-hud .jn-th{width:auto;padding:0 14px;gap:8px;justify-content:flex-start;flex:0 0 auto;}#jj-sc-hud .jn-st{width:56px;padding:0;justify-content:center;}#jj-sc-hud .jn-st svg{width:24px;height:23px;display:block;}' +
    ...
    '#jj-ach .tab span{position:relative;}#jj-ach .tab.on .ic .on{opacity:1;}#jj-ach .tab.on .ic .off{opacity:0;}#jj-ach .tab .ndot{position:absolute;right:5px;top:3px;width:9px;height:9px;border-radius:50%;background:#FF00F5;box-shadow:0 0 0 2px rgba(8,14,35,.82),0 0 10px #FF00F5;opacity:0;transform:scale(0);pointer-events:none;}#jj-ach .tab.new .ndot{opacity:1;transform:none;animation:jjTabPing 1.25s ease-in-out infinite;}@keyframes jjTabPing{0%,100%{opacity:.55;scale:.82;}50%{opacity:1;scale:1.18;}}' +
    ...
    /* rows */
    '#jj-ach .row{position:relative;display:grid;grid-template-columns:auto 1fr;gap:0 18px;align-items:center;padding:16px 18px;height:138px;min-height:138px;flex:0 0 auto;border-radius:16px;overflow:visible;box-sizing:border-box;transition:box-shadow .3s ease,border-color .3s ease;}' +
    ...
    '#jj-ach.t-classic .tab::after{content:"";position:absolute;left:14px;right:14px;bottom:-1px;height:3px;border-radius:3px;background:linear-gradient(90deg,#3b7dff,#b04cff);transform:scaleX(0);transition:transform .3s cubic-bezier(.22,1,.36,1);}#jj-ach.t-classic .tab.on{color:#fff;}#jj-ach.t-classic .tab.on::after{transform:scaleX(1);}#jj-ach.t-classic .tab:hover{color:#fff;background:linear-gradient(180deg,rgba(100,135,255,.12),rgba(176,76,255,.08));box-shadow:inset 0 -10px 20px rgba(120,90,255,.08);}#jj-ach.t-classic .tab:hover::after{transform:scaleX(.45);}#jj-ach.t-classic .tab.on:hover::after{transform:scaleX(1);}' +
    ...
    '#jj-ach.t-retro .tab.on{border-image-source:url(' + RETRO.cardW + ');color:#0b1e5a;}#jj-ach.t-retro .tab.on::before{content:"▶";color:#e0322b;font-size:10px;margin-right:2px;}#jj-ach.t-retro .tab:hover,#jj-ach.t-retro .tab.on:hover{transform:none;filter:brightness(1.15) drop-shadow(3px 3px 0 rgba(255,0,245,.8));}' +
    ...
    '#jj-ach .card{width:min(1200px,86vw);min-width:0;}' +
    ...
    '#jj-ach .row,#jj-ach.t-retro .row{height:138px;min-height:138px;padding:16px 72px 16px 18px;grid-template-columns:72px minmax(0,1fr);column-gap:18px;box-sizing:border-box;}' +
    '#jj-ach .row .ico,#jj-ach.t-retro .row .ico{width:72px;height:72px;box-sizing:border-box;}' +
    '#jj-ach .row .chk{right:18px;top:50%;bottom:auto;transform:translateY(-50%);}' +
    '#jj-ach.t-medieval .row.done,#jj-ach.t-mixed .row.done{border-width:1.5px;border-style:solid;border-image:none;border-radius:16px;padding:16px 72px 16px 18px;}' +
    '#jj-ach.t-medieval .row.done .rbg,#jj-ach.t-mixed .row.done .rbg{display:block;inset:-2px;border:20px solid transparent;border-image:url(' + IMG.frame + ') 96 / 20px / 0 round;border-radius:0;box-sizing:border-box;}' +
    '#jj-ach.t-mixed .row.done .rbg{background:rgba(10,18,36,.96);}' +
    ...
    'html[data-jj-theme] .jjst-ov .row{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));width:min(100%,620px);margin-inline:auto;}html[data-jj-theme] .jjst-ov .row .jjst-glass{width:100%;min-width:0;}' +
  ...
  function mount() {
    ...
    var storePeek = document.createElement('img'); storePeek.className = 'store-peek'; storePeek.src = GB + 'ach-alien-catch.png'; storePeek.alt = ''; storePeek.setAttribute('aria-hidden', 'true'); storePeek.addEventListener('error', function () { storePeek.hidden = true; }); hud.querySelector('.jn-st').prepend(storePeek);
  ...
  function payout(a, o) {
    ...
    floatAt(x, y - 18, (kind === 'star' ? '+1' : kind === 'coin' ? '+' + (a.reward || COIN) : 'Theme unlocked!') + ' <img src="' + ico(kind) + '" alt="">');
  ...
  function buildPanel() {
    ...
    panel.innerHTML = '<div class="card"><i class="bgd"></i><img class="banner l" src="' + IMG.bannerL + '" alt=""><img class="banner r" src="' + IMG.bannerR + '" alt=""><img class="peek" src="' + ALIEN_PEEK + '" alt="">' +
      ...
      '<div class="tabs">' + TABS.map(function (t) { var ic = GB + 'score-tab-' + TABICON[t[0]]; return '<div class="tab" role="tab" tabindex="0" data-tab="' + t[0] + '" data-cursor="hover"><i class="bb"></i><i class="ic"><img class="off" src="' + ic + '-off.webp" alt="" onerror="this.style.display=\'none\'"><img class="on" src="' + ic + '-on.webp" alt="" onerror="this.style.display=\'none\'"></i><span>' + t[1] + '</span><i class="ndot" aria-hidden="true"></i></div>'; }).join('') + '</div>' +
    ...
    panel.querySelectorAll('.tab').forEach(function (b) { var go = function (e) { e.preventDefault(); e.stopPropagation(); curTab = b.getAttribute('data-tab'); seeTab(curTab); renderRows(); };
  ...
  function renderRows() {
    ...
    listEl.innerHTML = rows.length ? rows.map(function (a) {
      ...
      var rw = a.kind === 'theme' ? '<img class="th" src="' + IMG.theme + '" alt=""><span>Theme</span>' : '<img src="' + ico(a.kind) + '" alt=""><span>' + (a.kind === 'star' ? 1 : (a.reward || COIN)) + '</span>';
